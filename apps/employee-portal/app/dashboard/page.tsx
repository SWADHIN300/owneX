  ...
  return (
    <main className="wrap">
      ...
      <div className="grid">
        <div className="card">
          <h3>Identity</h3>
          <p>{v.jobTitle ?? "Team member"}</p>
          <p className="muted">{v.department ?? "Northwind Industries"}</p>
        </div>
        <div className="card">
          <h3>Permissions</h3>
          <p>
            {v.permissions
              ? Object.entries(v.permissions)
                  .filter(([, x]) => x)
        ...
        <div className="card">
          <h3>Membership</h3>
          <p>{v.membership?.expired ? "Expired" : "Active"}</p>
          <p className="muted">
            Joined{" "}
            {v.membership?.joinedAt
              ? new Date(Number(v.membership.joinedAt) * 1000).toLocaleDateString()
              : "—"}
          </p>
        </div>
      </div>
